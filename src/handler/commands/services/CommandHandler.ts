...
export class CommandHandler {
  ...
  private static async checkCommandPermission(
    ...
    let reply: string;
    if (cooldown?.timeLeft && cooldown.timestamp) {
      // Format the timestamp for Discord's timestamp feature
      const timestamp = Math.floor(cooldown.timestamp);
      reply = config.deniedCommandReplies.cooldowns[cooldown.type]
        ?.replace(/{timestamp}/g, timestamp.toString());
    } else {
      reply = config.deniedCommandReplies.specific[reason ?? ''] || config.deniedCommandReplies.general;
    }
